package org.pfaa.geologica.client.render;

import net.minecraft.block.Block;
import net.minecraft.client.renderer.RenderBlocks;
import net.minecraft.world.IBlockAccess;

import org.pfaa.geologica.block.WallBlock;

public class CompositeWallBlockRenderer extends CompositeBlockRenderer {

	public CompositeWallBlockRenderer(int renderId) {
		super(renderId);
	}

	@Override
	protected void renderInventoryBlockPass(Block block, int metadata, RenderBlocks renderer) {
		WallBlock wallBlock = (WallBlock)block;
		wallBlock.enableRenderAsWall();
		renderer.renderBlockAsItem(block, metadata, 1.0F);
		wallBlock.disableRenderAsWall();
	}

	@Override
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z,
			Block block, int modelId, RenderBlocks renderer) {
		return renderer.renderBlockWall((WallBlock)block, x, y, z);
	}
	
}
