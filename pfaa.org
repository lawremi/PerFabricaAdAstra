...
*** Calcite
	...
	* Gypsum + clinker mixture is ground to cement
      * Concrete:
	* Railcraft and Chisel have concrete, but not cement
	* Need to mix 2x gravel + 1x sand + 1x cement + water
      * Thut's Mods adds liquid concrete that dries but noone uses it
      * May need to introduce notion of "synthetic mixture"
      * Can also produce cement by grinding slag, or by burning oil shale
      * Natural sources include volcanic ash, diatomite, calcined zeolite
      * Geopolymer cement: slag + coal fly ash + sodium silicate with
        >1.4 SiO2:NaO molar ratio; no cooking required
    * Quicklime (Witchery...) by roasting in a furnace
