...
** Ore Genesis
   A major issue in Minecraft is that mining consumes non-renewable
   resources. To sustain a steady stream of ores, one comes to rely on
   semi-automated mining methods, which present a host of problems and
   can never be fully automated.

   There needs to be a progression from manual mining and exploration
   to automated infrastructure that progresses in efficiency. To
   retain balance, infinite sources of resources should be finite in
   number and difficult or impossible to relocate. Consider Thaumcraft
   nodes, Buildcraft springs, and Botania Daybloom Primals.

   We propose a semi-realistic mini-game of ore generation, based on
   fixed sources of geological activty that when properly harnessed
   generate a steady supply of ore /in situ/.

   Perhaps the simplest approach would be bedrock-level springs of
   mineral-rich hot water that when cooled precipitates ore
   minerals. The type of mineral would depend on the proportion of
   ores originally in that chunk. More complicated would be springs of
   brines (or some solution) that need to be passed over/along stone
   to dissolve minerals and are finally cooled to convert adjacent
   stone to ore blocks. The fluid might act like water, and so could
   flow a long way after being pumped up, but it would need to be
   continuously heated (e.g., by adjacent lava) to retain its fluid
   gathering ability. The generated ore blocks could be repeatedly
   "broken" like those in Harder Ores, although we might tweak that
   mechanic so that spawning an ore chunk is stochastic, with the rate
   depending non-linearly on the ore concentration. 

   Another idea: the spring block generates a flowing block, not a
   source block. Any stone touched by the liquid is converted over
   time to an ore block, which is harvested as normal and must be
   replaced by another stone block for another ore to be generated. 
...
** Gregtech
   We will let GT manage the processing when it is present.  We will
   work with the maintainer to add new types of materials. Once a
   material is added, we can modify traits, such as the composition
   (including existing materials). These are then automatically mapped
   to machine recipes. For example, pulverizer support includes the
   IC2 macerator, TE pulverizer, and Railcraft rock crusher.
