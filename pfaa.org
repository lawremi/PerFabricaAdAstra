...
**** Steam cracking
     ...
     4. Ethene, propene and C4 olefins
        * C4 => butadiene + butene, separated by chemistry
          * butene itself is a mixture of isomers but often used
            directly
...
** Reactions
   ...
   We might not need to actually have a Reaction
...
** Processes
   ...
   some composition, and grinding will always output its primary
   component as a dust. Both the dust and the original ore block can
   ...
   is something we need anyway. But a major limitation is that an
   ItemStack should be registered under a single dictionary
   term. Another problem is that the equivalency only holds for
   specific processes (such as a reaction with a specific partner).
   Thus, we will need additional means of abstraction.

   For crude processing operations like smelting, we have a recipe
   registry proxy that maps operations on chemicals to operations on
   ores and minerals. For general mixing and reactions, however, it is
   not clear that there is always an equivalence between, e.g., an
   intermediate mixture containing a chemical and the chemical
   itself. Other chemicals in the mixture might also react, bringing
   about a conflict. We could simply ignore the conflict case. It
   might also lead to an unecessary explosion in recipes. We really
   only need the reactions that are important for processing the raw
   mixture. We could specify reactions with "pass through" reactants
   representing other mixture components.
