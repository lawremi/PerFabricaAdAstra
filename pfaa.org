...
**** Design
     ...
     (coke) from the top, and accept heated, compressed air (the blast)
     from the bottom, potentially with oil, natural gas, tar, powdered
     coal and/or oxygen to increase the heat. 
     
     The air is blown, optionally (for higher efficiency) heated by
     cowper furnances, and enters the furnace via special pipes called
     /tuyeres/, which are made of copper and cooled with water.
     ...
     * Uptake valves :: Top, through which dirty gas exits
     ...
     The structure must be cuboid. The larger, the more ore that can be
     smelted at once. The structural blocks might be bricks, with
     special "carbon" bricks at the bottom.

     A cowper stove is a simple regenerative heat exchanger, where the
     dirty, hot gases escaping the top of the furnace heat refractory
     bricks that then transfer their heat to convert the cold blast to
     a hot blast. There must be at least two (usually three) stoves,
     because there is only one gas flow. First, the top gas is passed
     through to heat the bricks, then the blast air.
...
**** Implementation
     ...
     * Centrifugal fan :: PneumaticCraft and RotaryCraft both have an
          air compressor that generates PC compressed air; those are
          not mainstream, so we may need our own machine, perhaps
          based on components from steam turbines.
     * Steam blower :: Somehow adapt steam turbines to push air with a
                       fan, instead of generating power
     * Notches :: For input/output, we could use e.g. Gregtech's
                  input/output hatches, or Railcraft's valves; their
                  function would depend on the position within the
                  multiblock.
     * Tuyeres :: This may need to be a custom block
...
** Multiblocks
   ...
   the world (and thus in 3D). Could have an API like the crafting
   API. A component block listens for neighbor updates and, upon an
   update, it checks for a candidate "master" block (this might be
   itself, or at some offset to itself, or it simply delegates the
   request to its neighbors). Once a master is identified, the system
   initializes a tile entity, which represents a high-level object
   that models the behavior of the machine as a whole.
